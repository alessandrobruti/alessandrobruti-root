...
# -*- coding: utf-8 -*-
"""
Genera la WBS Excel di un progetto di servizi a partire da una specifica JSON
ricavata dal contratto. Cinque fogli: WBS, Timeline, Riepilogo, Parametri, Ruoli.

    python3 build_wbs.py spec.json --out "NOME_FILE.xlsx"

Tutti i calcoli sono formule Excel vive: il file si ricalcola all'apertura.
Le giornate front office e di viaggio sono CALCOLATE da n. incontri × durata ×
n. persone; le giornate di back office sono l'unico input di effort, perché un
contratto non le quantifica mai.

Identità visiva: skill impresoft-brand. Palette di tre colori — giallo
istituzionale, nero, bianco — più grigi puri derivati dal nero. Nessun colore
semantico: gli stati usano la scala monocroma e le parole.

DUE TRAPPOLE DI OPENPYXL, già risolte qui, che costano mezz'ora a riscoprire:

1. I fill vanno dichiarati in ARGB a 8 cifre con start_color E end_color.
   openpyxl omette l'attributo colore quando coincide con il proprio default, e
   "000000" normalizzato È quel default: un fill nero dichiarato in forma
   abbreviata sparisce dai dxf della formattazione condizionale, e le barre nere
   del Gantt non vengono disegnate. Vedi la funzione fill().

2. Una fascia unita che copre una colonna in cui poi si scrive un totale rende
   quella cella una MergedCell, non scrivibile. La fascia del totale si ferma
   prima delle colonne numeriche.

E una di Excel: una cella con formula che restituisce "" NON è vuota, quindi
blocca lo sbordo del testo nelle celle adiacenti. Per questo la fascia dei mesi
usa sigle di tre lettere su ogni settimana invece di un nome intero centrato sul
gruppo, che resterebbe tagliato.
"""
import argparse
import json
import sys
...
ap.add_argument("spec", help="specifica JSON del progetto (vedi spec.esempio.json)")
ap.add_argument("--out", help="percorso del file da scrivere; default dalla spec")
args = ap.parse_args()
...
PAR = SPEC["parametri"]
FASI = SPEC["fasi"]
OUT = args.out or PRJ["file"]
TITOLO = PRJ["titolo"]
OFFERTA = PRJ.get("offerta", "contratto non indicato")

for i, f in enumerate(FASI, 1):
    mancanti = {"fase", "tipo", "mod", "blocco", "w0", "wd", "back"} - set(f)
    if mancanti:
        sys.exit(f"fase {i} ({f.get('fase','?')}): campi mancanti {sorted(mancanti)}")
    f.setdefault("inc", None); f.setdefault("ore", None); f.setdefault("pers", None)
    for k in ("descr", "prep", "follow", "inp", "outp", "owner", "part", "deliv", "note"):
        f.setdefault(k, None)

# ---------------------------------------------------------------- brand
# Font istituzionale. Se Manrope non è installato sulle macchine che apriranno
# il file, mettere "Arial" (fallback indicato dal manuale).
FONT = PAR.get("font", "Manrope")
...
def fill(hex6):
    """ARGB a 8 cifre con entrambi i colori — vedi trappola 1 nel docstring."""
    argb = "FF" + hex6
...
wb = Workbook()
wb.remove(wb.active)      # via il foglio "Sheet" di default
...
PARAMS = [
    ...
    (6,  "Ore per giornata", PAR["ore_giornata"], INT, "h",
     "Base di conversione giornate → ore."),
    (7,  "Tariffa oraria — Front office", PAR.get("tariffa_front"), EUR, "€/h",
     "Ore erogate al cliente, incontri in remoto inclusi."),
    (8,  "Tariffa oraria — Back office", PAR.get("tariffa_back"), EUR, "€/h",
     "Lavoro interno: analisi, delivery, reportistica."),
    (9,  "Tariffa oraria — Viaggio", PAR.get("tariffa_viaggio"), EUR, "€/h",
     "Tempo di trasferimento. Incluso nel monte ore di progetto."),
    (10, "Ore di viaggio per trasferta (a/r, per persona)", PAR.get("ore_viaggio_trasferta"), NUM, "h",
     PAR.get("nota_viaggio", "Stima sulla distanza fra le sedi.")),
    (SEZ, 12, "PROGETTO"),
    (13, "Lead time target di progetto", PAR.get("lead_time_target"), INT, "settimane",
     PAR.get("nota_lead_time", "Durata dichiarata nel contratto.")),
    (14, "Investimento contrattuale (una tantum)", PAR.get("investimento"), EUR, "€",
     "Comprende già i costi di trasferta degli incontri in presenza."),
    (15, "Anno di riferimento", PAR.get("anno"), INT, "anno",
    ...
    (18, "Km andata e ritorno per trasferta", PAR.get("km_trasferta"), INT, "km",
     PAR.get("nota_km", "Stima sulle sedi indicate nel contratto.")),
    (19, "Ammortamento ACI", PAR.get("aci_eur_km"), EUR, "€/km", " "),
    (20, "Pedaggi — casello", PAR.get("pedaggi"), EUR, "€/auto/trasferta", " "),
    (21, "Vitto", PAR.get("vitto"), EUR, "€/persona/trasferta", " "),
    (22, "Persone per auto", PAR.get("persone_auto"), INT, "n.",
     "Quante persone viaggiano insieme: determina il numero di viaggi auto."),
]
for row in PARAMS:
    if row[0] == SEZ:
        _, r, title = row
        banda(ps, r, "A", "E", f"›  {title}", FILL_YEL,
...
ps["C7"].comment = Comment("Da qui si ricalcola tutto il costo del progetto e il margine "
                           "nel foglio Riepilogo.", "WBS")
...
rs["A2"].comment = Comment(
    "Se sulla WBS le persone sono indicate con ruoli generici mentre il contratto nomina "
    "figure specialistiche, la differenza va segnalata: al cliente è stata promessa una "
    "competenza dedicata.", "WBS")

for i, ruolo in enumerate(SPEC.get("ruoli", [])):
    r = 3 + i
    rs[f"A{r}"] = ruolo.get("ruolo"); rs[f"A{r}"].font = F_BODYB; rs[f"A{r}"].alignment = TOP
    rs[f"B{r}"] = ruolo.get("nome")
    rs[f"B{r}"].font = F_IN; rs[f"B{r}"].fill = FILL_WHT
    rs[f"B{r}"].border = INBOX; rs[f"B{r}"].alignment = TOP
    rs[f"C{r}"] = ruolo.get("seniority"); rs[f"C{r}"].font = F_BODY; rs[f"C{r}"].alignment = CTRV
    rs[f"D{r}"] = ruolo.get("presidio"); rs[f"D{r}"].font = F_BODY; rs[f"D{r}"].alignment = TOP
    rs[f"E{r}"] = ruolo.get("note"); rs[f"E{r}"].font = F_BODY; rs[f"E{r}"].alignment = TOP
    for col in "ACDE":
        rs[f"{col}{r}"].border = HRULE
    rs.row_dimensions[r].height = 32

if SPEC.get("referenti_cliente"):
    r = 4 + len(SPEC.get("ruoli", []))
    rs[f"A{r}"] = f"Lato {PRJ.get('cliente', 'cliente')}"
    rs[f"A{r}"].font = F_BODYB
    rs[f"A{r+1}"] = SPEC["referenti_cliente"]
    rs[f"A{r+1}"].font = F_BODY; rs[f"A{r+1}"].alignment = TOP
    rs.merge_cells(f"A{r+1}:E{r+1}")
    rs.row_dimensions[r + 1].height = 30
...
banda_titolo(ws, 1, "A", LAST, f"WORK BREAKDOWN STRUCTURE — {TITOLO}")
...
# tante colonne quante servono a contenere il piano, con un po' di respiro
NW = max(12, max(f["w0"] + f["wd"] - 1 for f in FASI) + 2)
W_FIRST = 7
...
c = ts["F2"]
c.value = PAR["settimana_iso_partenza"]
...
line(31, "Investimento contrattuale (una tantum)", f"={P_INV}", EUR, "€",
     "Le trasferte degli incontri in presenza sono già dentro questa cifra.")
...
line(47, "Lead time target dichiarato", f"={P_TGT}", INT, "settimane", "Durata dichiarata nel contratto.")
...
sm["C49"] = ('=IF(C45=C47,"Il piano quadra con il lead time target.",'
             '"Scostamento sul lead time dichiarato: verificare le settimane nella WBS.")')
